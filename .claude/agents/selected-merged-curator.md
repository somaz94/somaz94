...
description: 'Curates the hand-maintained `### Selected merged work` highlight table in this repo''s `README.md` — the marquee block of MERGED pull requests sitting ABOVE the `<!-- OSS:START -->` marker (everything INSIDE those markers is regenerated daily by `scripts/oss_contributions.py` and must never be hand-edited). Verifies each candidate is actually MERGED via `gh pr view`, keeps the reel to 12–16 rows (hard cap 18), and balances it for variety. Use when the user says "update the highlight table / 대표 머지 추가 / selected work 갱신", or after a notable external PR merges. Edits only that block; never commits.'
